...
search_year, search_motor, search_price = search_criteria

search_result = {}
for car in car_data.items():
    name, characteristics = car
    color, year, motor, carcase, price = characteristics
    if year >= search_year and motor >= search_motor and price <= search_price:
...
people_records.insert(0, ('Jeka', 'Lichman', 32, 'QA', 'Dnipro'))
